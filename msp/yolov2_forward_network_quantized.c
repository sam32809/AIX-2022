...
int max_abs(int src, int max_val)
{
    ...
    else if (abs(src) < abs(max_val)) src = src;
    return src;
}
float max_abs_float(float src, float max_val)
{
    if (fabs(src) > fabs(max_val)) src = (src > 0) ? max_val : -max_val;
    return src;
}
...
void forward_convolutional_layer_q(layer l, network_state state)
{
    ...
    for (z = 0; z < l.inputs; ++z) {
        ...
        //int q_src = round(src *((pow(2, 7) - 1) / 780));
    ...
    for (j = 0; j < net.n; ++j) {
        ...
        */
        ...
        if (l->type == CONVOLUTIONAL) { // Quantize conv layer only
            ...
            // Weight Quantization        
            l->weights_quant_multiplier = 49; // Arbitrarily set to 32; you should devise your own method to calculate the weight multiplier

            //find scale weight max, min value
            float min_w = 127.;
            float max_w = -min_w;
            for (fil = 0; fil < l->n; ++fil) {
                for (i = 0; i < filter_size; ++i) {
                    float ws = l->weights[fil * filter_size + i] * l->weights_quant_multiplier; // Scale=Sw=51
                    if (max_w < ws) {
                        max_w = ws;
                    }
                    if (min_w > ws) {
            ...
            //fixed max_weight and min_weight; fix range of fp
            //int max_fixed = 379;
            //int min_fixed = -377;
            int max_fixed = 607;
            int min_fixed = -603;
            int diff_fixed = max_fixed - min_fixed;
            printf("max of fp=%d\nmin of fp=%d\n", max_fixed, min_fixed);

            int z_w = -round(min_w * ((pow(2,8)-1)/diff_fixed)) - 128;
            printf("z_w=%d\n",z_w);
            ...
            for (fil = 0; fil < l->n; ++fil) {
                for (i = 0; i < filter_size; ++i) {
                    float ws = l->weights[fil * filter_size + i] * l->weights_quant_multiplier ; // Scale=Sw
                    int q_w = round(ws);
                    //printf("q_w=%d\n", q_w);
                    l->weights_int8[fil * filter_size + i] = max_abs(q_w, MAX_VAL_8); // Clip
                    if (max_quant_w <= l->weights_int8[fil * filter_size + i]) {
                        max_quant_w = l->weights_int8[fil * filter_size + i];
                    }
                    if (min_quant_w >= l->weights_int8[fil * filter_size + i]) {
                        min_quant_w = l->weights_int8[fil * filter_size + i];
                    }
                }
            }
            printf("max_quant_w=%d\nmin_quant_w=%d\n", max_quant_w, min_quant_w);


            // Bias Quantization
            float min_b = 32767.;
            float max_b = -min_b;
            int min_quant_b = 32768;
            int max_quant_b = -32768;
            float biases_multiplier = (l->weights_quant_multiplier * l->input_quant_multiplier);
            for (fil = 0; fil < l->n; ++fil) {
                float bs = l->biases[fil] * biases_multiplier; // Scale=Sb=SwxSin=1224
                if (max_b < bs) {
                    max_b = bs;
                }
                if (min_b > bs) {
            ...
            //int max_fixed_bias = 1387;
            //int min_fixed_bias = -759;
            //int z_b = -round(min_b * (pow(2, 8) - 1) / (max_fixed_bias - min_fixed_bias)) - pow(2,7);
           // printf("z_b=%d\n", z_b);

            for (fil = 0; fil < l->n; ++fil) {
                 float bs = l->biases[fil] * biases_multiplier ; // Scale=Sb=SwxSin
                 int q_b = floor(bs + z_w);
                 l->biases_quant[fil] = max_abs(q_b, MAX_VAL_16); // Clip
                 //printf("quant_bias=%d\n", l->biases_quant[fil]);
                 if (max_quant_b <= l->biases_quant[fil]) {
                     max_quant_b = l->biases_quant[fil];
                 }
                 if (min_quant_b >= l->biases_quant[fil]) {
                     min_quant_b = l->biases_quant[fil];
                 }
            }
            printf("max_quant_b=%d\nmin_quant_b=%d\n", max_quant_b, min_quant_b);
