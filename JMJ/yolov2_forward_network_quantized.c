    ...
    for (j = 0; j < net.n; ++j) {
        ...
        */
        ...
        if (l->type == CONVOLUTIONAL) { // Quantize conv layer only
            ...
            //int max_fixed_bias = 1000;
            //int min_fixed_bias = -3000;
            //int z_b = -round(min_w * (pow(2, 8) - 1) / (max_fixed_bias - min_fixed_bias)) - pow(2,7);
            //printf("z_b=%d\n", z_b);

            for (fil = 0; fil < l->n; ++fil) {
                 float bs = l->biases[fil] * biases_multiplier ; // Scale=Sb=SwxSin
                 int q_b = round(bs);
                 l->biases_quant[fil] = max_abs(q_b+z_w, MAX_VAL_16); // Clip
                 //printf("quant_bias=%d\n", l->biases_quant[fil]);
